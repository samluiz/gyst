...
class SqlDelightMigrationTest {
    ...
    fun migrateFromV1ToLatestAppliesHardeningAndDedup() {
        ...
        assertTrue(categoryColumns.contains("recurrence_series_id"))

        val scheduleColumns =
            driver.executeQuery(
                identifier = null,
                sql = "PRAGMA table_info(payment_schedule_item)",
                mapper = { cursor ->
                    QueryResult.Value(
                        buildMap {
                            while (cursor.next().value) {
                                put(cursor.getString(1).orEmpty(), cursor.getLong(3) == 1L)
                            }
                        },
                    )
                },
        ...
        assertEquals(true, scheduleColumns["category_id"])
        ...
        val statements =
            listOf(
                ...
                "INSERT INTO category(id, name, type, color, icon) " +
                    "VALUES ('cat-1', 'Assinaturas', 'FIXED', NULL, NULL)",
                "INSERT INTO subscription(id, name, amount_cents, billing_day, category_id, active, " +
                    "renewal_policy, next_due_date) VALUES " +
                    "('sub-1', 'Netflix', 1000, 10, 'cat-1', 1, 'MONTHLY', '2026-01-10')",
                "INSERT INTO budget_month(id, year_month, total_income_cents, created_at) " +
                    "VALUES ('b1', '2026-01', 100000, '2026-01-01T00:00:00Z')",
                "INSERT INTO budget_month(id, year_month, total_income_cents, created_at) " +
                    "VALUES ('b2', '2026-01', 120000, '2026-01-02T00:00:00Z')",
                "INSERT INTO payment_schedule_item(id, kind, ref_id, due_date, amount_cents, status, paid_at) " +
                    "VALUES ('s1', 'SUBSCRIPTION', 'sub-1', '2026-01-10', 1000, 'DUE', NULL)",
                "INSERT INTO payment_schedule_item(id, kind, ref_id, due_date, amount_cents, status, paid_at) " +
                    "VALUES ('s2', 'SUBSCRIPTION', 'sub-1', '2026-01-10', 1000, 'DUE', NULL)",
            )
